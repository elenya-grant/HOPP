...
class PEM_H2_Clusters_2022:
    ...
    Stack characteristics are aligned with HFTO PEM 2022 Status technical targets
        ...
        - stack life of 40,000 hours

    """
    #Remove: estimate_lifetime_capacity_factor
    #Remove: make_lifetime_performance_df_all_opt [x]
    def __init__(self, cluster_size_mw, plant_life, eol_eff_percent_loss=10,uptime_hours_until_eol = 40000, include_degradation_penalty=True,turndown_ratio=0.1,dt=3600):
        ...
        self.cell_active_area = 1949 #1920#1250 #[cm^2]
        self.N_cells = 135 #130
        self.membrane_thickness=0.0077#0.0125 #cm
    ...
    def calc_reversible_cell_voltage(self,Stack_T):
        ...
        #Buck is more accurate! Antoine has more error as lower temp (https://www.omnicalculator.com/chemistry/vapour-pressure-of-water#vapor-pressure-formulas)
...
if __name__ == "__main__":
    ...
    electrolyzer_model_parameters = {
        ...
        "uptime_hours_until_eol": 40000, #for calculating steady deg rate
        "include_degradation_penalty": True,
        "turndown_ratio":0.1,
    }
    # Create PEM and initialize parameters
    pem = PEM_H2_Clusters_2022(cluster_size_mw, plant_life,**electrolyzer_model_parameters)
    
    # pem.membrane_thickness = 0.0125
    # pem.membrane_thickness = 0.0077
    # pem.cell_active_area = 1949
    # pem.N_cells = 135
    # pem.cell_active_area = 1920#1250 #[cm^2]
    # pem.N_cells = 130
    pem.make_BOL_efficiency_curve()
    i_max = 2*pem.cell_active_area
    nf = pem.faradaic_efficiency(i_max)
    V_rated=pem.cell_design(pem.T_C,i_max)
    h2_rated = pem.h2_production_rate(i_max,1)
    power = i_max*V_rated*pem.N_cells/1e3
    eff_rated = power/h2_rated
    ...
    # pem.output_dict['BOL Efficiency Curve Info']["Current Density"] = pem.output_dict['BOL Efficiency Curve Info']["Current"]/pem.cell_active_area
    
    # V_rated = pem.output_dict['BOL Efficiency Curve Info']['Cell Voltage'].max()
    # i_rated = pem.output_dict['BOL Efficiency Curve Info']['Current'].max()
    # H2_rated = pem.output_dict['BOL Efficiency Curve Info']['H2 Produced'].max()
    # eff_rated = pem.output_dict['BOL Efficiency Curve Info']["Efficiency [kWh/kg]"].max()
    # p_rated = pem.output_dict['BOL Efficiency Curve Info']['Power Consumed [kWh]'].max()
    j_rated = i_max/pem.cell_active_area
    
    print(f"\n {power:.2f} kW: {j_rated:.2f} A/cm2 @ {V_rated:.4f} V/cell and {eff_rated:.2f} kWh/kg {pem.N_cells} cells with {pem.cell_active_area} cm2 cell area")
