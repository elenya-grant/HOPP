...
class PEM_H2_Clusters_2026:
    ...
    Stack characteristics are aligned with HFTO PEM 2022 Status technical targets
        ...
        - 3 A/cm2 at 1.8 V/cell
        - 48 kWh/kg electrical efficiency
        - stack life of 80,000 hours

    """
    #Remove: estimate_lifetime_capacity_factor
    #Remove: make_lifetime_performance_df_all_opt [x]
    def __init__(self, cluster_size_mw, plant_life, eol_eff_percent_loss=10,uptime_hours_until_eol = 80000, include_degradation_penalty=True,turndown_ratio=0.1,dt=3600):
        ...
        self.cell_active_area = 1425 #1920#1250 #[cm^2]
        self.N_cells = 130
        self.membrane_thickness=0.0166#0.018 #cm
        self.cell_max_current_density = 3 #[A/cm^2]
    ...
    def calc_reversible_cell_voltage(self,Stack_T):
        ...
        pcathode_atm=1 #1 #[atm] total pressure at the cathode
    ...
    def calc_V_act(self,Stack_T,I_stack,cell_active_area):
        ...
        i_o_c = 10**-3 #2 * (10 ** (-3))  #
        # V_anode = (((self.R * T_K) / (a_a * z_a * self.F)) * np.arcsinh(i / (2 * i_o_a)))
        # V_cathode= (((self.R * T_K) / (a_c * z_c * self.F)) * np.arcsinh(i / (2 * i_o_c)))
        V_anode = (((self.R * T_K) / (a_a * z_a * self.F)) * np.arcsinh(i / (1 * i_o_a)))
        V_cathode= (((self.R * T_K) / (a_c * z_c * self.F)) * np.arcsinh(i / (1 * i_o_c)))
...
if __name__ == "__main__":
    ...
    pem = PEM_H2_Clusters_2026(cluster_size_mw, plant_life,**electrolyzer_model_parameters)
    ...
    pem.membrane_thickness = 0.0166
    pem.cell_active_area = 1425 #1500
    pem.cell_max_current_density = 3
    pem.T_C = 80
    pem.max_cell_current=pem.cell_max_current_density*pem.cell_active_area
    I_stack = pem.cell_max_current_density*pem.cell_active_area

    V_target = 1.8

    E_cell = pem.calc_reversible_cell_voltage(pem.T_C)

    V_allowed = V_target - E_cell
    V_act = pem.calc_V_act(pem.T_C,I_stack,pem.cell_active_area)
    V_ohmic=pem.calc_V_ohmic(pem.T_C,I_stack,pem.cell_active_area,pem.membrane_thickness)
    V_cell = E_cell + V_act + V_ohmic
    print("V_cell = {}".format(V_cell))
    h2 = pem.h2_production_rate(I_stack,1)
    power_kWh = V_cell*pem.N_cells*I_stack/1e3
    print("efficiency: {} kWh/kg".format(round(power_kWh/h2,3)))
    print("rated power {} kW".format(power_kWh))
    []

    # pem.curve_coeff=pem.iv_curve()
    # pem.make_BOL_efficiency_curve()
    # power,h2 = pem.rated_h2_prod()
    # pem.output_dict['Efficiency [kWh/kg]'] = pem.output_dict['BOL Efficiency Curve Info']['Power Consumed [kWh]']/pem.output_dict['BOL Efficiency Curve Info']['H2 Produced']
    []
